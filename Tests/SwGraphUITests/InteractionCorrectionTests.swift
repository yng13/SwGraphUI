...
@Suite struct InteractionCorrectionTests {
    ...
    /// Test for Parent Origin=center, Child Origin=topLeft | 親 Origin=center, 子 Origin=topLeft のテスト
    @Test func mixedOriginParentCenterChildTopLeft() async throws {
        ...
        // Parent's top-left edge should be (50, 50) (100 - 100*0.5) | 親の左上端は (50, 50) になるはず (100 - 100*0.5)
        ...
        // Child's absolute coordinate (top-left edge) is resolved with evaluateAbsolutePosition | 子の絶対座標 (左上端) は evaluateAbsolutePosition で解決
    ...
    /// Coordinate conversion test in a 3-level hierarchy (Parent -> Child -> Grandchild) | 親 -> 子 -> 孫 の 3 段階階層での座標変換テスト
    @Test func nestedAbsoluteRelativeConversion() async throws {
        ...
        // 1. Confirm conversion to absolute coordinate | 1. 絶対座標への変換確認
        ...
        // 2. Confirm inverse conversion from absolute to relative coordinate | 2. 絶対座標から相対座標への逆変換確認
        // Converting (160, 160) back to relative coordinate relative to the grandchild's parent (child) should be (10, 10) | (160, 160) を孫の親（child）基準の相対座標に戻すと (10, 10) になるはず
    ...
    /// Verification of drag offset and post-movement relative coordinate in hierarchical structure | 階層構造におけるドラッグオフセットと移動後の相対座標の検証
    @Test func hierarchicalDragOffset() async throws {
        ...
        var state = DragState()
        // Click near the center (175, 175) of the child (abs=150, 150) to start dragging | 子(abs=150,150) の中心付近 (175, 175) をクリックしてドラッグ開始
        state.startDrag(nodes: [child], nodeLookup: lookup, pointer: .init(x: 175, y: 175))
        
        #expect(state.draggedNodes[0].distance.x == 25) // 175 - 150
        
        // Drag to (200, 200) | (200, 200) までドラッグ
        ...
        // New absolute coordinate is 200 - 25 = 175 | 新しい絶対座標は 200 - 25 = 175
        // Relative coordinate seen from parent (100, 100) is 175 - 100 = 75 | 親(100,100) から見た相対座標は 175 - 100 = 75
        #expect(results["c"]?.x == 75)
    }
    
    /// Test for circular reference guard (Confirm it still works after coordinate conversion refactor) | 循環参照ガードのテスト (座標変換リファクタ後も機能することを確認)
    ...
    /// Confirm that the deprecated Convenience Overload works in flat graphs | 非推奨の Convenience Overload が単一階層（Flat Graph）で動作することを確認
    @Test func deprecatedConvenienceStillWorksForFlatGraph() async throws {
        ...
        // Calling startDrag (added nodeLookup) | startDrag の呼び出し (nodeLookup を追加)
    ...
    /// Test for fitView including hierarchical nodes | 階層ノードを含む fitView のテスト
    @Test func fitViewWithHierarchicalNodes() async throws {
        ...
        // Bounds are from (1000, 1000) to (1100, 1100) = size 100x100 | Bounds は (1000,1000) から (1100,1100) = size 100x100
        #expect(viewport.zoom == 6.0)
    }

    /// Test for dimension fallback | 寸法フォールバックのテスト
