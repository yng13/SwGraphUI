...
@Suite struct InteractionTests {
    ...
    // 1. Relative positioning of multiple nodes should be maintained during dragging | 1. ドラッグ中に複数ノードの相対配置が維持されること
    ...
    // 2. Snap should not break even when the result is clamped by Extent (Snap -> Extent) | 2. Extent 適用時に clamp された結果でも snap が破綻しないこと (Snap -> Extent)
    @Test func snapThenExtentOrdering() async throws {
        ...
        // SnapGrid: 20, Extent: 0...50, Size: 10x10 | SnapGrid: 20, Extent: 0...50, Size: 10x10
        // If pointer is at 45: | ポインタが 45 にある場合:
        // 1. Snap -> 40 | 1. Snap -> 40
        // 2. Extent (0...40) -> 40 | 2. Extent (0...40) -> 40
        ...
        // If pointer is at 55: | ポインタが 55 にある場合:
        // 1. Snap -> 60 | 1. Snap -> 60
        // 2. Extent (0...40) -> 40 (Clamp wins) | 2. Extent (0...40) -> 40 (Clamp wins)
    ...
    // 3. zoom at point should preserve the graph point corresponding to the specified screen point | 3. zoom at point で、指定 screen point に対応する graph point が保存されること
    @Test func zoomAtPointInvariance() async throws {
        ...
        // Calculate graphPoints corresponding to screenPoint before zoom | ズーム前の screenPoint に対応する graphPoints を計算
        ...
        // Recalculate graphPoints corresponding to screenPoint after zoom | ズーム後の screenPoint に対応する graphPoints を再計算
    ...
    // 4. fitView should not break with an empty node array | 4. fitView が空ノード配列で壊れないこと
    @Test func fitViewHandlesEmptyNodesSafely() async throws {
        ...
        // Default values should be returned | デフォルト値が返されること
