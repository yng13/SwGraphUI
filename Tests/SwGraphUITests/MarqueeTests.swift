...
final class MarqueeTests: XCTestCase {
    ...
    func testMarqueeSelection() {
        ...
        // 1. Node placement (graph space) | 1. ノードの配置 (グラフ空間)
        // Node A: (10, 10), Size: (50, 50) | Node A: (10, 10), Size: (50, 50)
        // Node B: (100, 100), Size: (50, 50) | Node B: (100, 100), Size: (50, 50)
        ...
        // 2. Viewport setup (zoom 1.0, no offset) | 2. ビューポートの設定 (拡大率1.0, オフセットなし)
        store.setViewport(Viewport(x: 0, y: 0, zoom: 1.0))
        
        // 3. Marquee start and update (graph space) | 3. Marquee の開始と更新 (グラフ空間)
        store.startMarquee(at: CGPoint(x: 0, y: 0))
        store.updateMarquee(to: CGPoint(x: 70, y: 70))
        
        // 4. Marquee end | 4. Marquee の終了
    ...
    func testMarqueeWithViewportOffset() {
        ...
        // Viewport setup | ビューポート設定
        store.setViewport(Viewport(x: -50, y: -50, zoom: 2.0))
        
        // 1. Partially overlapping selection (intersects is true, but contains is false) | 1. 部分的に重なる選択 (intersects は true だが contains は false)
        ...
        // 2. Fully enclosed selection (graph space from (40, 40) to (160, 160)) | 2. 完全に囲む選択 (グラフ空間の 40,40 から 160,160)
        store.startMarquee(at: CGPoint(x: 40, y: 40))
