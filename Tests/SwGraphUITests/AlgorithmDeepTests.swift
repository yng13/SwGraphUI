...
@Suite struct GeometryAlgorithmsTests {
    @Test func getViewportForBoundsWithAsymmetricPaddingStrict() async throws {
        ...
        // Asymmetric padding (top: 10, left: 50, bottom: 100, right: 200) | 非対称パディング (top: 10, left: 50, bottom: 100, right: 200)
        ...
        // Calculate and verify actual applied padding (P2 support: ensure all directions are satisfied) | 実際の適用パディングを計算して検証 (P2対応: 全方位の充足確認)
        ...
        // Ensure all paddings are at least the specified value | 全てのパディングが指定値以上であることを確認
        #expect(applied.left >= 50)
        #expect(applied.top >= 10)
        #expect(applied.right >= 200)
        #expect(applied.bottom >= 100)
        
        // Due to asymmetric padding correction, at least one side should match (or be very close to) the padding value | 非対称パディング補正により、少なくとも一つの辺がパディング値と一致（または極めて近い）はず
...
@Suite struct EdgePathAlgorithmsTests {
    @Test func smoothStepSameSideNearDistanceCoordinateCheck() async throws {
        // Verify smoothStepPoints directly (P2 support: elimination of dependency on SVG strings) | smoothStepPoints を直接検証 (P2対応: SVG文字列依存の排除)
        ...
        // sourceDir=(1,0), offset=20, diff=10 -> gapOffsetVal=10, sourceGapOffset=(-10,0) | sourceDir=(1,0), offset=20, diff=10 -> gapOffsetVal=10, sourceGapOffset=(-10,0)
        // gappedSource = (20 - 10, 0) = (10, 0) | gappedSource = (20 - 10, 0) = (10, 0)
        #expect(points[1].x == 10.0)
        #expect(points[1].y == 0.0)
        
        // targetGapped = (10+20, 50) = (30, 50) | targetGapped = (10+20, 50) = (30, 50)
        // Verify that overlap is avoided | 重なりが回避されていることを検証
    ...
    @Test func smoothStepMixedHandlesCoordinateCheck() async throws {
        ...
        let points = result.points
        // Verify Source (Right) progression direction | Source(Right) 進展方向の確認
        #expect(points[1].x > points[0].x)
        #expect(points[1].y == points[0].y)
        
        // Verify Target (Bottom) entry direction | Target(Bottom) 進入方向の確認
